...
--slicing outputs from data set
y = data:narrow(2,1,1)

--creating biased inputs (equation y = a + b x1 + c x2)
X = torch.cat(torch.ones(data:size(1),1),data:narrow(2,2,2))
biasedDataTest = torch.cat(torch.ones(dataTest:size(1),1),dataTest)

XT = X:t()

LSParams = torch.inverse(XT*X)*XT*y

for i=1,dataTest:size(1) do
  local prediction = LSParams:t()*biasedDataTest[i]
  print("Prediction: ", prediction[1], " text: ", text[i])
end
