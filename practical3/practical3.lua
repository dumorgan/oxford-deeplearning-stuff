...
for i = 1, iterations do
  ...
  if i % 10 == 0 then -- don't print *every* iteration, this is enough to get the gist
    ...
    testLoss = criterion:forward(test_outputs, test.labels)
    testLosses[#testLosses + 1] = testLoss
    print(string.format("minibatches processed: %6s, loss = %6.6f, testLoss = %6.6f", i, minibatch_loss[1], testLoss))
  end
...
gnuplot.plot({
  ...
  {
    torch.linspace(1, #losses, #testLosses),
...
local winnerProbability, classPredictions = torch.max(classProbabilities, 2)
-- classPredictions holds predicted classes from 1-10

local rights = 0
local testSize = classPredictions:size(1)
-- TODO: compute test classification error here for the second handin item
for i = 1,testSize do
  rights = rights + (classPredictions[i][1] == test.labels[i] and 1 or 0)
end
print("Classification error: "..100*(testSize - rights)/testSize.."%")
